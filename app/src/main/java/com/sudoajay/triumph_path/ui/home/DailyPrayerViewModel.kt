...
//    fun getPagingGsonSourceWithDataBase(): Flow<PagingData<DailyPrayerDB>> {
...
//    }
        ...
        fun getAllDataFromDataBase(): Flow<DailyPrayerDB> {
            val database = DailyPrayerDatabase.getDatabase(viewModelApplication)
        val dailyPrayerRepository = DailyPrayerRepository(database.dailyPrayerDoa())
            return dailyPrayerRepository.getAllData(getTodayDate()).asFlow()
        }
